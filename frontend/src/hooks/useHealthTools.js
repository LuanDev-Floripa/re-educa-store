...
/**
 * useHealthTools
 * - Cálculos de saúde (IMC, calorias, macros, idade biológica, hidratação)
 * - Retornos padronizados com fallbacks em erro
 */
...
export const useHealthTools = () => {
  ...
  const calculateCalories = useCallback(
    async (age, gender, weight, height, activityLevel, goal) => {
      try {
        setLoading(true);
        setError(null);

        const response = await apiClient.calculateCalories(
          age,
          gender,
          weight,
          height,
  ...
  const calculateBiologicalAge = useCallback(
    async (age, gender, healthData) => {
      try {
        setLoading(true);
        setError(null);

        const response = await apiClient.calculateBiologicalAge(
          age,
          gender,
          healthData,
        );
  ...
  const calculateHydration = useCallback(
    async (weight, activityLevel, climate) => {
      try {
        setLoading(true);
        setError(null);

        const response = await apiClient.calculateHydration(
          weight,
          activityLevel,
          climate,
        );
