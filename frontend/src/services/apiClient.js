...
import { getAuthToken, getRefreshToken, setAuthToken, setRefreshToken, clearAllTokens } from '../utils/storage';
import logger from '../utils/logger';

// Garante que a URL sempre termina com /api/v1
const getApiBaseUrl = () => {
  const envUrl = import.meta.env.VITE_API_URL;
  if (!envUrl) {
    throw new Error("VITE_API_URL não está configurado no .env");
  }
  // Remove /api e /api/v1 se já existir para evitar duplicação
  const baseUrl = envUrl.replace(/\/api(\/v1)?\/?$/, "");
  return `${baseUrl}/api/v1`;
};

const API_BASE_URL = getApiBaseUrl();
// Construir WS_URL a partir de VITE_WS_URL ou VITE_API_URL
const getWsUrl = () => {
  ...
  if (apiUrl) {
    const base = apiUrl.replace(/\/api(\/v1)?\/?$/, "").replace(/^https?/, "wss");
    return `${base}/ws`;
  }
  // Fallback apenas para desenvolvimento
  return import.meta.env.PROD ? "wss://api.topsupplementslab.com/ws" : "ws://localhost:9001/ws";
};
...
class ApiClient {
  ...
  getHeaders(includeAuth = true, isFormData = false) {
    ...
    // Só adiciona Content-Type se não for FormData (o browser define automaticamente)
    if (!isFormData) {
      headers["Content-Type"] = "application/json";
    }

    // Atualiza token antes de cada requisição para pegar mudanças
    this.token = getAuthToken() || this.token;
   ...
   * Faz requisição POST
   * @param {string} endpoint Caminho relativo começando com '/'
   * @param {RequestInit & { includeAuth?: boolean, body?: string | object }} [options]
   * @returns {Promise<any>} Corpo JSON já parseado
   */
  ...
  async handleResponse(response) {
    ...
    // Retorna no formato { data, ... } para compatibilidade
    return { data, ...data };
  }
  ...
  async refreshAuthToken() {
    ...
    this.clearTokens();
    // Disparar evento customizado para navegação reativa
    const navigateEvent = new CustomEvent('navigate', { detail: { path: '/login' } });
    window.dispatchEvent(navigateEvent);
    // Fallback para navegação não reativa apenas se evento não for tratado
    setTimeout(() => {
      if (!window.__navigationHandled) {
        window.location.href = "/login";
      }
      window.__navigationHandled = false;
    }, 100);
    return false;
  }
  ...
  setTokens(token, refreshToken) {
    ...
    setAuthToken(token, true); // true para manter compatibilidade com 'token' legado
