...
export const GamificationSystem = ({ 
  ...
  const defaultGamificationData = useMemo(() => ({
    ...
    levels: [
      { level: 1, experience: 0, title: 'Iniciante', color: 'text-gray-600', icon: Star },
      { level: 2, experience: 100, title: 'Novato', color: 'text-green-600', icon: Star },
      { level: 3, experience: 250, title: 'Aprendiz', color: 'text-blue-600', icon: Star },
      { level: 4, experience: 500, title: 'Intermediário', color: 'text-purple-600', icon: Award },
      { level: 5, experience: 750, title: 'Avançado', color: 'text-orange-600', icon: Award },
      { level: 6, experience: 1000, title: 'Experiente', color: 'text-red-600', icon: Award },
      { level: 7, experience: 1500, title: 'Expert', color: 'text-yellow-600', icon: Trophy },
      { level: 8, experience: 2000, title: 'Mestre', color: 'text-yellow-600', icon: Trophy },
      { level: 9, experience: 2500, title: 'Lenda', color: 'text-purple-600', icon: Crown },
      { level: 10, experience: 3000, title: 'Ícone', color: 'text-purple-600', icon: Diamond }
    ]
  }), []);
          ...
          <div className="flex items-center space-x-4 mb-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                Nível {gamificationData.user?.level} • {gamificationData.user?.tier?.toUpperCase()}
              </p>
              <Button 
                size="sm" 
                variant="outline" 
              ...
              <div className="mt-3">
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span>{gamificationData.user?.experience} / {gamificationData.user?.nextLevel}</span>
                </div>
                <Progress 
                  value={getProgressPercentage(gamificationData.user?.experience, gamificationData.user?.nextLevel)} 
                  className="h-2"
                />
              </div>
            </div>
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="text-right">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-sm font-medium">{quest.coins}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Gem className="w-4 h-4 text-purple-600" />
                        <span className="text-sm font-medium">{quest.gems}</span>
                      </div>
                    </div>
                  </div>
                ...
                <div key={challenge.id} className="p-4 border rounded-lg">
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    ...
                    <div className="text-xs text-gray-500">
                      Expira em {new Date(challenge.endDate).toLocaleDateString()}
                    </div>
                  </div>
                </div>
              );
        ...
        {gamificationData.rewards?.map((reward) => {
          ...
          return (
            <Card key={reward.id} className="hover:shadow-lg transition-shadow">
              <CardContent className="p-4">
                <div className="flex items-center space-x-3 mb-3">
                  <div className={`p-2 ${getRarityBgColor(reward.rarity)} rounded-lg`}>
        ...
        <CardContent>
          <div className="space-y-4">
            {gamificationData.leaderboard?.map((player, index) => {
              ...
              return (
                <div key={player.id} className={`flex items-center space-x-4 p-4 border rounded-lg ${getTierBgColor(player.tier)}`}>
