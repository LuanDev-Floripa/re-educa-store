...
const DashboardReal = ({ 
  ...
  const getWidgetBgColor = (type) => {
    switch (type) {
      ...
      case 'progress': return 'bg-indigo-50';
      ...
      <CardContent>
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <span className="font-semibold">{dashboardData?.stats?.exercises_today || 0}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-600">Calorias queimadas</span>
            <span className="font-semibold">{dashboardData?.stats?.calories_burned || 0}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-600">Tempo ativo</span>
            <span className="font-semibold">{dashboardData?.stats?.active_time || '0min'}</span>
          </div>
        </div>
      </CardContent>
    </Card>
      ...
      <CardContent>
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <span className="font-semibold">{dashboardData?.health?.imc || 'N/A'}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-600">Peso</span>
            <span className="font-semibold">{dashboardData?.health?.weight || 'N/A'}kg</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm text-gray-600">Meta de calorias</span>
            <span className="font-semibold">{dashboardData?.health?.calorie_goal || 'N/A'}</span>
          </div>
        </div>
      </CardContent>
    </Card>
  ...
  const renderWidget = (widget) => {
    switch (widget.type) {
      case 'welcome': return renderWelcomeWidget();
      case 'stats': return renderStatsWidget();
      case 'goals': return renderGoalsWidget();
      case 'achievements': return renderAchievementsWidget();
      case 'health': return renderHealthWidget();
            ...
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
              ...
              <div className="space-y-2">
                {[
                  ...
                  return (
                    <label key={widget.id} className="flex items-center space-x-3 p-2 hover:bg-gray-50 rounded">
                      <input
                        type="checkbox"
