...
export const PersonalizedDashboard = ({
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      if (token) {
        try {
          ...
          if (response) {
            ...
            // Widgets podem ser criados a partir dos dados do dashboard
            // Por enquanto, manter widgets padrão ou criar dinamicamente
            if (response.widgets && Array.isArray(response.widgets)) {
              setWidgets(response.widgets);
            } else {
              // Criar widgets dinamicamente a partir dos dados
              const dynamicWidgets = [
                {
                  id: "health-score",
                  type: "metric",
                  title: "Health Score",
                  value: apiDashboardData.healthScore,
                  icon: Activity,
                  color: "text-primary",
                },
                {
                  ...
                  data: apiDashboardData.weeklyGoals,
                  icon: Target,
                  color: "text-primary",
                },
                {
                  ...
                  data: apiDashboardData.recentActivities.slice(0, 5),
                  icon: Clock,
                  color: "text-primary",
                },
                {
                  ...
                  type: "grid",
                  title: "Conquistas",
                  data: apiDashboardData.achievements.slice(0, 6),
                  icon: Trophy,
                  color: "text-primary",
                },
              ];
          ...
          logger.warn("Erro ao carregar dashboard da API, usando fallback:", apiError);
        }
      }
      ...
      setDashboardData(defaultDashboardData); // Garantir que sempre há dados
      setWidgets(defaultWidgets);
    } finally {
      setLoading(false);
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4" role="status" aria-label="Carregando dashboard">
