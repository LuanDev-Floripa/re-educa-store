...
export const PersonalizedDashboard = ({ 
  ...
  const defaultDashboardData = useMemo(() => ({
    ...
    goals: [
      { id: 1, name: 'Perda de Peso', target: 70, current: 75, unit: 'kg', progress: 75, deadline: '2024-06-01' },
      { id: 2, name: 'Ganho de Massa', target: 70, current: 65, unit: 'kg', progress: 65, deadline: '2024-12-01' },
      { id: 3, name: 'Redução de Gordura', target: 12, current: 15, unit: '%', progress: 80, deadline: '2024-08-01' }
    ],
    recentWorkouts: [
      { id: 1, name: 'Treino de Força', date: '2024-01-28', duration: 60, exercises: 8, calories: 450, type: 'strength' },
      { id: 2, name: 'Cardio HIIT', date: '2024-01-27', duration: 30, exercises: 5, calories: 300, type: 'cardio' },
      { id: 3, name: 'Yoga', date: '2024-01-26', duration: 45, exercises: 12, calories: 200, type: 'flexibility' }
    ],
    achievements: [
    ...
    recommendations: [
      { id: 1, name: 'Whey Protein Premium', type: 'product', reason: 'Baseado no seu objetivo de ganho de massa', confidence: 95 },
      { id: 2, name: 'Supino Reto', type: 'exercise', reason: 'Perfeito para seu nível intermediário', confidence: 88 },
      { id: 3, name: 'Ganho de Massa Avançado', type: 'workout_plan', reason: 'Alinhado com seus objetivos', confidence: 90 }
    ],
    weather: {
      temperature: 25,
      condition: 'sunny',
  ...
  const getWidgetIcon = (type) => {
    switch (type) {
      ...
      case 'recent_workouts': return Activity;
  ...
  const getWidgetColor = (type) => {
    switch (type) {
      ...
      case 'stats': return 'text-green-600';
      case 'goals': return 'text-purple-600';
      case 'recent_workouts': return 'text-orange-600';
      case 'achievements': return 'text-yellow-600';
      case 'recommendations': return 'text-pink-600';
      case 'nutrition': return 'text-red-600';
      case 'health': return 'text-cyan-600';
      case 'weather': return 'text-indigo-600';
  ...
  const getWidgetBgColor = (type) => {
    switch (type) {
      case 'welcome': return 'bg-blue-50';
      case 'stats': return 'bg-green-50';
      case 'goals': return 'bg-purple-50';
      case 'recent_workouts': return 'bg-orange-50';
      case 'achievements': return 'bg-yellow-50';
      case 'recommendations': return 'bg-pink-50';
      case 'nutrition': return 'bg-red-50';
      case 'health': return 'bg-cyan-50';
      case 'weather': return 'bg-indigo-50';
    ...
    <Card className={`h-full ${getWidgetBgColor('welcome')}`}>
      <CardContent className="p-6">
        ...
        <div className="mt-4">
          ...
          <Progress 
            value={(dashboardData.user?.experience / dashboardData.user?.nextLevel) * 100} 
            className="h-2"
          />
        </div>
      </CardContent>
      ...
      <CardContent>
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className="font-semibold">{dashboardData.stats?.totalWorkouts}</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-semibold">{Math.round(dashboardData.stats?.totalDuration / 60)}h</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-semibold">{dashboardData.stats?.totalCalories.toLocaleString()}</span>
          </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-semibold">{dashboardData.stats?.currentStreak} dias</span>
          </div>
        </div>
      </CardContent>
    </Card>
            ...
            <div key={rec.id} className="p-3 border rounded-lg">
              ...
              <div className="flex items-center justify-between">
                <Badge variant="outline" className="text-xs">
                  {rec.type === 'product' ? 'Produto' :
                   rec.type === 'exercise' ? 'Exercício' : 'Plano'}
                </Badge>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="font-semibold">
              {dashboardData.nutrition?.today?.calories} / {dashboardData.nutrition?.goals?.calories}
            </span>
          </div>
          <Progress 
            value={(dashboardData.nutrition?.today?.calories / dashboardData.nutrition?.goals?.calories) * 100} 
            className="h-2"
          />
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-semibold">
              {dashboardData.nutrition?.today?.protein}g / {dashboardData.nutrition?.goals?.protein}g
            </span>
          </div>
          <Progress 
            value={(dashboardData.nutrition?.today?.protein / dashboardData.nutrition?.goals?.protein) * 100} 
            className="h-2"
          />
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-semibold">
              {dashboardData.nutrition?.today?.water}L / {dashboardData.nutrition?.goals?.water}L
            </span>
          </div>
          <Progress 
            value={(dashboardData.nutrition?.today?.water / dashboardData.nutrition?.goals?.water) * 100} 
            className="h-2"
          />
        </div>
      </CardContent>
      ...
      <CardContent>
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <span className="font-semibold">{dashboardData.health?.bmr} kcal</span>
          </div>
          
          <div className="flex items-center justify-between">
            <span className="text-sm">TDEE</span>
            <span className="font-semibold">{dashboardData.health?.tdee} kcal</span>
          </div>
          
          <div className="flex items-center justify-between">
            <span className="text-sm">Gordura Corporal</span>
            <span className="font-semibold">{dashboardData.health?.bodyFat}%</span>
          </div>
          
          <div className="flex items-center justify-between">
            <span className="text-sm">Massa Muscular</span>
            <span className="font-semibold">{dashboardData.health?.muscleMass} kg</span>
          </div>
        </div>
      </CardContent>
    </Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {defaultWidgets.map((widget) => {
                  ...
                  const isVisible = widgets.find(w => w.id === widget.id)?.visible;
                  ...
                  return (
                    <div
                      ...
                      onClick={() => handleWidgetUpdate(widget.id, { visible: !isVisible })}
                    >
                      <div className="flex items-center space-x-3">
                        <IconComponent className={`w-5 h-5 ${getWidgetColor(widget.type)}`} />
                        <div>
                        ...
                        {isVisible && <Check className="w-5 h-5 text-blue-600 ml-auto" />}
                      </div>
                    </div>
                  );
                })}
