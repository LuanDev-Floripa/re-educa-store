...
import { setSocialRecentSearches } from "@/utils/storage";
/**
...
const SocialSearch = ({
  ...
  useEffect(() => {
    ...
    const loadRecentSearches = async () => {
      const { getSocialRecentSearches } = await import("@/utils/storage");
      const savedSearches = getSocialRecentSearches();
  ...
  const loadTrendingHashtags = async () => {
    try {
      ...
      try {
        ...
        // API de posts não disponível, usando hashtags padrão
      }

      // Fallback para hashtags padrão
      setTrendingHashtags(commonHashtags);
    } catch (err) {
      logger.error("Erro ao carregar trending hashtags:", err);
  ...
  const handleSearch = (query = searchQuery) => {
    ...
    setSocialRecentSearches(updatedSearches);
  };
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center space-x-2 mb-1">
              ...
              {user.is_verified && <Check className="h-4 w-4 text-primary" />}
            </div>
