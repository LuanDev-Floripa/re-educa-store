...
export const UserProfile = ({ 
  ...
  const defaultProfileData = useMemo(() => ({
    ...
    achievements: [
      { id: 1, name: 'Primeiro Treino', date: '2023-01-20', icon: Trophy, color: 'text-primary' },
      { id: 2, name: '100 Treinos', date: '2023-06-15', icon: Award, color: 'text-primary' },
      { id: 3, name: 'Meta de Peso', date: '2023-09-10', icon: Target, color: 'text-primary' },
      { id: 4, name: '1 Ano de Treino', date: '2024-01-15', icon: Crown, color: 'text-primary' }
    ],
    workoutHistory: [
  ...
  // Função para obter cor de fundo do tier (para uso em style inline quando necessário)
  const getTierBgColorValue = (tier) => {
    switch (tier) {
      case 'bronze': return 'hsl(var(--primary))';
      case 'silver': return 'hsl(var(--muted-foreground))';
      case 'gold': return 'hsl(var(--primary))';
      case 'platinum': return 'hsl(var(--primary))';
  ...
  const getTierBgColor = (tier) => {
    switch (tier) {
      case 'bronze': return 'bg-primary/10';
      case 'silver': return 'bg-muted';
      case 'gold': return 'bg-primary/10';
    ...
    <div className="space-y-6">
      ...
      <Card>
        <div className="relative">
          <div className="h-40 sm:h-48 bg-gradient-to-r from-primary to-primary/80 rounded-t-lg">
            <div className="absolute inset-0 bg-black/20 rounded-t-lg"></div>
            <div className="absolute bottom-4 left-4 right-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-end space-y-4 sm:space-y-0 sm:space-x-4">
                <div className="relative">
                  <div className="w-20 h-20 sm:w-24 sm:h-24 bg-card rounded-full border-4 border-card shadow-lg">
                    <img
                  ...
                  {profileData.isVerified && (
                    <div className="absolute -bottom-1 -right-1 w-5 h-5 sm:w-6 sm:h-6 bg-primary rounded-full flex items-center justify-center border-2 border-card">
                ...
                <div className="flex-1 text-primary-foreground">
                  ...
                  <p className="text-primary-foreground/90 mb-2 text-sm sm:text-base">{profileData.bio}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-foreground">{new Date(profileData.birthDate).toLocaleDateString()}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <User className="w-4 h-4 text-muted-foreground" />
                  <span className="capitalize text-foreground">{profileData.gender}</span>
                </div>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                <div className="flex items-center space-x-2">
                  <div className={`w-3 h-3 rounded-full ${profileData.isPremium ? 'bg-primary' : 'bg-muted-foreground'}`}></div>
                  <span className="text-sm text-foreground">{profileData.isPremium ? 'Premium' : 'Básico'}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <Clock className="w-4 h-4 text-muted-foreground" />
                  <span className="text-sm text-foreground">Última atividade: {new Date(profileData.lastActive).toLocaleDateString()}</span>
                </div>
                <div className="flex items-center space-x-2">
                  <Shield className="w-4 h-4 text-muted-foreground" />
                  <span className="text-sm text-foreground">Conta verificada</span>
                </div>
              </div>
            </div>
          </div>
              ...
              <div key={index} className="p-4 border rounded-lg">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div>
                    ...
                    <div className="font-semibold text-foreground">{nutrition.protein}g</div>
                  </div>
                  <div>
                    <div className="text-muted-foreground">Carboidratos</div>
                    <div className="font-semibold text-foreground">{nutrition.carbs}g</div>
                  </div>
                  <div>
                    <div className="text-muted-foreground">Gordura</div>
                    <div className="font-semibold text-foreground">{nutrition.fat}g</div>
                  </div>
                  <div>
                    <div className="text-muted-foreground">Água</div>
                    <div className="font-semibold text-foreground">{nutrition.water}L</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {Object.entries(settings.notifications || {}).map(([key, value]) => (
            <div key={key} className="flex items-center justify-between p-3 border rounded-lg">
              <div>
                <h3 className="font-semibold capitalize text-foreground">{key.replace(/([A-Z])/g, ' $1')}</h3>
                <p className="text-sm text-muted-foreground">
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4" role="status" aria-label="Carregando perfil">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex items-center space-x-2">
                ...
                <div 
                  ...
                  style={{ backgroundColor: getTierBgColorValue(profileData.tier) }}
                ></div>
              </div>
            </CardContent>
          </Card>
