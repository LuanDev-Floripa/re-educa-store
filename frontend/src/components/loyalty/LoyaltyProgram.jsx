...
export const LoyaltyProgram = ({ 
  ...
  const loyaltyProgramData = {
    ...
    pointsHistory: [
      { date: '2024-01-10', action: 'Compra', points: 150, description: 'Whey Protein Premium' },
      { date: '2024-01-08', action: 'Avaliação', points: 10, description: 'Avaliou produto' },
      { date: '2024-01-05', action: 'Indicação', points: 100, description: 'Indicou amigo' },
      { date: '2024-01-03', action: 'Compra', points: 200, description: 'Creatina + BCAA' },
      { date: '2024-01-01', action: 'Bônus', points: 50, description: 'Bônus de Ano Novo' }
    ],
    availableRewards: [
      {
        id: 1,
    ...
    referralProgram: {
      ...
      referredFriends: currentUserProfile.referredFriends,
      totalEarned: 500,
      pendingRewards: 200,
      rewards: [
        { friend: 'Maria Santos', date: '2024-01-08', points: 100, status: 'completed' },
        { friend: 'Pedro Costa', date: '2024-01-05', points: 100, status: 'completed' },
        { friend: 'Ana Silva', date: '2024-01-03', points: 100, status: 'completed' },
        { friend: 'Carlos Lima', date: '2024-01-01', points: 100, status: 'completed' },
        { friend: 'Julia Oliveira', date: '2023-12-28', points: 100, status: 'completed' }
      ]
    }
  };
  ...
  const getTierProgress = () => {
    ...
    const nextTier = loyaltyData.tiers?.find(tier => tier.level === currentTier.level + 1);
    
    if (!nextTier) return 100; // Último tier
    
    const currentPoints = currentTier.points;
    const tierStart = loyaltyData.tiers?.find(tier => tier.level === currentTier.level)?.minPoints || 0;
  ...
  const renderOverview = () => (
    <div className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <div className="font-semibold">
                {loyaltyData.currentTier?.nextTierPoints?.toLocaleString()} pontos
              </div>
            </div>
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Benefícios do Nível {loyaltyData.currentTier?.name}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
        ...
        <CardContent>
          <div className="space-y-4">
            {loyaltyData.tiers?.map((tier) => {
              ...
              const isUnlocked = loyaltyData.currentTier?.points >= tier.minPoints;
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-3">
                      {React.createElement(getTierIcon(tier.id), { className: `w-6 h-6 ${getTierColor(tier.id)}` })}
                      <div>
            ...
            <CardContent className="p-4">
              ...
              {challenge.completed && (
                <div className="mt-3 flex items-center space-x-2 text-green-600">
                  ...
                  <span className="text-sm font-medium">Desafio Concluído!</span>
                </div>
              )}
            </CardContent>
          </Card>
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Compartilhe seu código e ganhe pontos quando seus amigos se cadastrarem
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center space-x-2">
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
        <div className="text-center">
          ...
          <p className="text-gray-600 dark:text-gray-400">Carregando programa de fidelidade...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          ...
          <Button 
            variant="outline" 
            size="sm"
            onClick={() => handleShareAchievement({ tier: loyaltyData.currentTier, points: loyaltyData.currentPoints })}
          >
