...
const CalorieCalculatorReal = ({ 
  ...
  const { calculateCalories, calculateMacros, loading, error } = useHealthTools();
  ...
  const handleCalculate = async () => {
    ...
    try {
      ...
      if (calorieResult.success) {
        ...
        if (showMacros) {
          const macroResult = await calculateMacros(calorieResult.data.calories, formData.goal);
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-gray-600">
            Calcule suas necessidades calóricas diárias baseadas em seus dados pessoais
          </p>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="flex justify-center">
            <Button 
              ...
              disabled={loading || !formData.age || !formData.gender || !formData.weight || !formData.height || !formData.activityLevel || !formData.goal}
              className="px-8"
            >
              {loading ? (
                <>
            ...
            <div className="mt-8 space-y-6">
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-center">
                  ...
                  <h3 className="text-lg font-semibold text-green-800">Resultado do Cálculo</h3>
                </div>
              </div>
                ...
                <Card>
                  <CardContent className="p-4 text-center">
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Multiplicador: {getActivityMultiplier(formData.activityLevel)}x
                    </p>
                  </CardContent>
                </Card>
              </div>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="text-center">
                        ...
                        <h4 className="font-semibold text-lg">{macros.protein}g</h4>
                        <p className="text-sm text-gray-600">Proteínas</p>
                        <p className="text-xs text-gray-500">{macros.protein_percentage}%</p>
                        <Progress value={macros.protein_percentage} className="mt-2" />
                      </div>
                      ...
                      <div className="text-center">
                        ...
                        <h4 className="font-semibold text-lg">{macros.carbs}g</h4>
                        <p className="text-sm text-gray-600">Carboidratos</p>
                        <p className="text-xs text-gray-500">{macros.carbs_percentage}%</p>
                        <Progress value={macros.carbs_percentage} className="mt-2" />
                      </div>
                      ...
                      <div className="text-center">
                        ...
                        <p className="text-xs text-gray-500">{macros.fat_percentage}%</p>
                        <Progress value={macros.fat_percentage} className="mt-2" />
                      </div>
                    </div>
                  </CardContent>
                </Card>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                    <div>
                      <p><strong>Nível de Atividade:</strong> {getActivityDescription(formData.activityLevel)}</p>
                      <p><strong>Objetivo:</strong> {getGoalDescription(formData.goal)}</p>
                    </div>
                    <div>
                      <p><strong>Fórmula Utilizada:</strong> {result.formula || 'Harris-Benedict'}</p>
