...
export const WorkoutPlanCard = ({ plan, onStartPlan, onViewDetails, onAddToFavorites }) => {
  const getGoalIcon = (goal) => {
    switch (goal) {
      ...
      case 'muscle_gain': return <Dumbbell className="w-4 h-4" />;
          ...
          {plan.image_url ? (
            ...
            <div className="w-full h-full flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-sm font-medium text-blue-600 dark:text-blue-400">{plan.name}</p>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-1">
            ...
            <span className="text-xs text-gray-500">
              ({plan.participants_count || 0} participantes)
            </span>
          </div>
          ...
          <div className="mt-4 pt-4 border-t">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm text-gray-500">
                {plan.progress.completed_workouts}/{plan.progress.total_workouts} treinos
              </span>
            </div>
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
              <div 
                className="bg-blue-600 h-2 rounded-full transition-all duration-300"
                style={{ width: `${(plan.progress.completed_workouts / plan.progress.total_workouts) * 100}%` }}
              ></div>
            </div>
          </div>
        )}
