...
const APIStatusDashboard = () => {
  ...
  const testGemini = async () => {
    try {
      const response = await fetch('/api/admin/ai/configs/gemini-real-config/test', {
        method: 'POST',
        headers: {
  ...
  const testPerplexity = async () => {
    try {
      const response = await fetch('/api/admin/ai/configs/perplexity-real-config/test', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('admin_token') || ''}`,
          'Content-Type': 'application/json'
  ...
  const runAllTests = useCallback(async () => {
    ...
    try {
      ...
      const allConnected = Object.values(newStatus).every(s => s === 'connected');
      if (allConnected) {
            ...
            <div className="flex items-center justify-between p-3 border rounded-lg">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Servidor Flask</p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between p-3 border rounded-lg">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Endpoints protegidos</p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between p-3 border rounded-lg">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">API de Pesquisa</p>
                </div>
              </div>
