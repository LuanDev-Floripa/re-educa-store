            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold flex items-center">
                  ...
                  <span className="ml-2 capitalize">{healthData?.status || 'Unknown'}</span>
                </p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{healthData?.available_configs || 0}</p>
              ...
              {configs.map((config) => (
                <Card key={config.id} className="border-l-4 border-l-blue-500">
                  <CardContent className="p-4">
                    ...
                    <h3 className="font-semibold mb-1">{config.service_name}</h3>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              { name: 'Google Gemini', provider: 'gemini', status: 'available' },
              { name: 'Perplexity', provider: 'perplexity', status: 'available' },
              { name: 'OpenAI', provider: 'openai', status: 'coming_soon' },
              { name: 'Claude', provider: 'claude', status: 'coming_soon' }
            ].map((item) => (
              <div key={item.provider} className="text-center p-4 border rounded-lg">
                <div className="text-3xl mb-2">{getProviderIcon(item.provider)}</div>
