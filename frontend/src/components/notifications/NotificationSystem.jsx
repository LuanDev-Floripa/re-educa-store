...
const NotificationSystemComponent = ({
  ...
  const loadNotifications = useCallback(async () => {
    ...
    try {
      ...
      setNotifications(data.notifications || data.data?.notifications || defaultNotifications);
    } catch (error) {
  ...
  const loadSettings = useCallback(async () => {
    try {
      ...
      const data = await apiClient.request("/user/settings");
      setSettings(data.settings || data.data?.settings || defaultSettings);
    } catch (error) {
    ...
    return (
      <Card
        ...
        className={`cursor-pointer transition-all hover:shadow-md ${
          !notification.isRead ? "ring-2 ring-primary bg-primary/10" : ""
        }`}
      ...
      >
        <CardContent className="p-4">
          <div className="flex items-start gap-4">
            <div
              className={`p-3 ${getNotificationBgColor(notification.type)} rounded-lg`}
            >
              <IconComponent
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {Object.entries(settings.general || {}).map(([key, value]) => (
            <div
              key={key}
              className="flex items-center justify-between p-4 border border-border/30 rounded-lg transition-colors duration-200 hover:bg-muted/30"
            >
              <div>
              ...
              <input
                ...
                className="w-5 h-5 text-primary rounded border-border/50 focus:ring-primary transition-all duration-200"
              />
            </div>
          ))}
        </CardContent>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto mb-4" role="status" aria-label="Carregando notificações">
