...
const DataExportReal = ({ 
  ...
  const fetchExportHistory = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setExportHistory(data.exports || data.data || []);
      } else {
        setExportHistory([]);
      }
    } catch (err) {
  ...
  const fetchScheduledExports = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        setScheduledExports(data.scheduled || data.data || []);
      } else {
        setScheduledExports([]);
      }
    } catch (err) {
  ...
  const dataTypes = [
    { id: 'profile', name: 'Perfil', icon: User, description: 'Dados pessoais e configurações' },
    { id: 'exercises', name: 'Exercícios', icon: Activity, description: 'Histórico de exercícios e treinos' },
    { id: 'goals', name: 'Metas', icon: Target, description: 'Objetivos e progresso' },
    { id: 'achievements', name: 'Conquistas', icon: Trophy, description: 'Conquistas e badges' },
    { id: 'health', name: 'Saúde', icon: Heart, description: 'Dados de saúde e medições' },
    { id: 'stats', name: 'Estatísticas', icon: BarChart3, description: 'Estatísticas e relatórios' }
  ];
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          ...
          <div>
            <h3 className="text-lg font-semibold mb-4">Selecione os dados para exportar</h3>
                ...
                return (
                  <div
                    ...
                    className={`flex items-center justify-between p-4 rounded-lg ${
                      getStatusBgColor(export_.status)
                    }`}
                  >
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-4 text-sm text-gray-600">
                          ...
                          <div className="flex items-center space-x-1">
                            ...
                            <span>{new Date(export_.createdAt).toLocaleDateString('pt-BR')}</span>
                          </div>
                    ...
                    <div className="flex items-center space-x-2">
                      <Badge variant={
                        ...
                        {export_.status === 'completed' ? 'Concluída' :
                         export_.status === 'processing' ? 'Processando' :
                         export_.status === 'failed' ? 'Falhou' : 'Cancelada'}
                      </Badge>
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center space-x-4 text-sm text-gray-600">
                          ...
                          <span>•</span>
                          <span>Próxima: {new Date(schedule.nextRun).toLocaleDateString('pt-BR')}</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center space-x-2">
