...
const ProductReviewsComponent = ({ productId, productName }) => {
  ...
  const loadReviews = useCallback(async () => {
    ...
    try {
      ...
      // Carregar avaliações reais da API usando apiService
      try {
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      ...
      if (editingReviewId) {
        ...
        data = await apiService.products.updateReview(productId, editingReviewId, {
        ...
        setReviews((prev) =>
          prev.map((review) =>
            review.id === editingReviewId ? { ...review, ...(data.review || data) } : review,
          ),
        );
      } else {
        // Criar nova avaliação
        data = await apiService.products.addReview(productId, {
          rating: newReview.rating,
          title: newReview.title,
          comment: newReview.comment,
          pros: newReview.pros,
      ...
      alert(editingReviewId ? "Avaliação atualizada com sucesso!" : "Avaliação enviada com sucesso!");
    } catch (error) {
  ...
  const handleHelpful = async (reviewId, isHelpful) => {
    try {
      const data = await apiService.products.voteReviewHelpful(productId, reviewId, isHelpful);
      ...
      logger.error("Erro ao votar em avaliação:", error);
    }
  };
  ...
  const handleDeleteReview = async (reviewId) => {
    ...
    try {
      await apiService.products.deleteReview(productId, reviewId);
      setReviews((prev) => prev.filter((review) => review.id !== reviewId));
    } catch (error) {
      logger.error("Erro ao deletar avaliação:", error);
      alert(error.message || "Erro ao deletar avaliação");
    }
  };
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-2">
                      {user && (review.userId === user.id || review.user_id === user.id) && (
                        <>
                          <Button
                            variant="ghost"
                            size="sm"
                  ...
                  {(review.pros || review.cons) && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      {review.pros && (
                        <div>
                          <h6 className="font-medium text-primary mb-1">
                            Pontos Positivos:
                          </h6>
                          <p className="text-sm text-muted-foreground">
                            {review.pros}
                      ...
                      {review.cons && (
                        <div>
                          <h6 className="font-medium text-destructive mb-1">
                            Pontos Negativos:
                          </h6>
                          <p className="text-sm text-muted-foreground">
                            {review.cons}
                    ...
                    <div className="flex items-center space-x-4">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        >
                          ...
                          {review.helpful || review.helpful_count || 0}
                        </Button>
                        <Button
                          variant="ghost"
                          size="sm"
                        ...
                        >
                          ...
                          {review.notHelpful || review.not_helpful_count || 0}
                        </Button>
                      </div>
                    </div>
                  </div>
...
// ProductReviews pode ter muitos reviews, otimização importante
