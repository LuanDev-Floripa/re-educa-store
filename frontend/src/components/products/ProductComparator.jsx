 ...
 */
const ProductComparatorComponent = ({
  products = [],
  onAddToCart,
  onAddToFavorites,
  onRemoveProduct,
  ...
  const loadProductsFromAPI = useCallback(async () => {
    try {
      const data = await apiService.products.getFeatured();
      ...
      if (productsList.length > 0) {
        setSelectedProducts(productsList.slice(0, maxProducts));
        setShowComparison(true);
      }
    } catch (error) {
  ...
  const getValueColor = (value, field, products) => {
    ...
    if (bestValue === null) return "text-foreground";
              ...
              <thead>
                <tr className="border-b border-border">
                  ...
                  {productsToCompare.map((product) => (
                    <th
                      key={product.id}
                      className="text-center py-3 px-4 font-medium text-foreground min-w-[200px]"
                    >
                      <div className="space-y-1">
                        ...
                        return (
                          ...
                          >
                            ...
                            {value ===
                              ...
                              <Check className="w-4 h-4 text-primary inline-block ml-1" />
                            )}
                          </td>
                        );
                      })}
...
// ProductComparator pode ter cálculos complexos, otimização importante
export const ProductComparator = memo(ProductComparatorComponent);
