...
const AIConfigPage = () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          setTestResults(prev => ({ ...prev, [configId]: { error: data.error } }));
  ...
  const testAllAPIs = async () => {
    try {
      ...
      // Testar Gemini
      const geminiResponse = await fetch('/api/admin/ai/configs/gemini-real-config/test', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      // Testar Perplexity
      const perplexityResponse = await fetch('/api/admin/ai/configs/perplexity-real-config/test', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      if (results.gemini === 'connected' && results.perplexity === 'connected') {
  ...
  const getAPIStatus = (configId) => {
    ...
    if (status === 'connected') {
      return { label: 'Conectado', variant: 'default', icon: Wifi, color: 'text-green-500' };
    } else if (status === 'error') {
      return { label: 'Erro', variant: 'destructive', icon: WifiOff, color: 'text-red-500' };
    } else {
      return { label: 'Não testado', variant: 'secondary', icon: AlertCircle, color: 'text-gray-500' };
    }
  };
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
          ...
          <div className="flex space-x-2">
            <Button variant="outline" onClick={testBasicConnectivity} disabled={loading}>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Select value={newConfig.provider} onValueChange={(value) => setNewConfig(prev => ({ ...prev, provider: value }))}>
                      <SelectTrigger>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => setNewConfig(prev => ({ ...prev, service_name: e.target.value }))}
                      ...
                      <div className="flex items-center space-x-4">
                        <div>
                          <div className="flex items-center space-x-2">
                            ...
                            <Badge variant={apiStatus.variant} className="flex items-center">
                              <APIStatusIcon className={`h-3 w-3 mr-1 ${apiStatus.color}`} />
                              {apiStatus.label}
                            </Badge>
                            {config.is_default && (
                          ...
                          <h3 className="font-semibold mt-2">{config.service_name}</h3>
                          <p className="text-sm text-muted-foreground">
                            {config.model_name} • {config.max_tokens} tokens • Temp: {config.temperature}
                          </p>
                          {config.api_endpoint && (
                          ...
                          {testResults[config.id] && (
                            <div className="mt-2 p-2 bg-muted rounded-md">
                              <p className="text-xs text-muted-foreground">
                                Último teste: {testResults[config.id].status || 'N/A'}
                              </p>
                              {testResults[config.id].response && (
                                <p className="text-xs text-muted-foreground">
                                  Resposta: {testResults[config.id].response.substring(0, 50)}...
                                </p>
                              )}
                            </div>
                          )}
                    ...
                    {showKeys[config.id] && (
                      <div className="mt-4 p-3 bg-muted rounded-md">
                        ...
                        <p className="font-mono text-sm break-all">
                          {config.is_mock ? 'mock-key-***' : 'encrypted-key-***'}
                        </p>
                      </div>
                    )}
                  </CardContent>
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Badge variant={apiStatus.backend === 'connected' ? 'default' : 'destructive'}>
                ...
                <Card>
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <Badge variant={
                        apiStatus.gemini === 'connected' && apiStatus.perplexity === 'connected' 
                          ? 'default' 
                          : 'destructive'
                      }>
                        {apiStatus.gemini === 'connected' && apiStatus.perplexity === 'connected' 
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex items-center space-x-2">
                      <Wifi className={`h-4 w-4 ${apiStatus.backend === 'connected' ? 'text-green-500' : 'text-red-500'}`} />
                  ...
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex items-center space-x-2">
                      <Wifi className={`h-4 w-4 ${apiStatus.ai_routes === 'connected' ? 'text-green-500' : 'text-red-500'}`} />
                      <span className="text-sm">Rotas de IA</span>
                    </div>
                    <Badge variant={apiStatus.ai_routes === 'connected' ? 'default' : 'destructive'}>
                      {apiStatus.ai_routes === 'connected' ? 'Conectado' : 'Desconectado'}
                    </Badge>
                  </div>
                  
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex items-center space-x-2">
                      <Wifi className={`h-4 w-4 ${apiStatus.gemini === 'connected' ? 'text-green-500' : 'text-red-500'}`} />
                      <span className="text-sm">Google Gemini</span>
                    </div>
                    <Badge variant={apiStatus.gemini === 'connected' ? 'default' : 'destructive'}>
                      {apiStatus.gemini === 'connected' ? 'Conectado' : 'Desconectado'}
                    </Badge>
                  </div>
                  
                  <div className="flex items-center justify-between p-3 border rounded-lg">
                    <div className="flex items-center space-x-2">
                      <Wifi className={`h-4 w-4 ${apiStatus.perplexity === 'connected' ? 'text-green-500' : 'text-red-500'}`} />
                      <span className="text-sm">Perplexity</span>
                    </div>
                    <Badge variant={apiStatus.perplexity === 'connected' ? 'default' : 'destructive'}>
                      {apiStatus.perplexity === 'connected' ? 'Conectado' : 'Desconectado'}
                    </Badge>
                  </div>
                </div>
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <Alert>
                ...
                <AlertDescription>
                  As estatísticas de uso estarão disponíveis quando as chaves reais forem configuradas.
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <Alert>
                <AlertCircle className="h-4 w-4" />
                <AlertDescription>
                  As configurações de segurança estão ativas e funcionando em modo mock.
                  Em produção, todas as chaves serão criptografadas automaticamente.
                </AlertDescription>
              </Alert>
            </CardContent>
          </Card>
