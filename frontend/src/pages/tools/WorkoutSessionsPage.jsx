...
const WorkoutSessionsPage = () => {
  ...
  const loadWorkoutHistory = async () => {
    try {
      ...
      setWorkoutHistory(data.entries || []);
    } catch (error) {
    ...
    try {
      ...
      await request(() => 
        apiService.health.addExerciseEntry({
          exercise_name: activeWorkout.name,
          duration: sessionData.totalTime,
          intensity: activeWorkout.difficulty,
          calories_burned: sessionData.caloriesBurned,
          exercise_type: 'workout',
          notes: `Treino completo: ${sessionData.exercisesCompleted} exercícios, ${sessionData.setsCompleted} séries`
        })
      );
      ...
      const newSession = {
        ...
        setsCompleted: sessionData.setsCompleted,
  ...
  const getStats = () => {
    ...
    const totalTime = workoutHistory.reduce((sum, session) => sum + session.duration, 0);
    const totalCalories = workoutHistory.reduce((sum, session) => sum + session.caloriesBurned, 0);
    const avgRating = workoutHistory.reduce((sum, session) => sum + session.rating, 0) / totalWorkouts || 0;
                  ...
                  <CardContent className="p-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        ...
                        <div>
                          <h4 className="font-semibold text-lg">{session.workoutName}</h4>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400">Queime 1000 calorias</p>
                      </div>
                    </div>
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400">Treine por 7 dias seguidos</p>
                      </div>
                    </div>
                  </div>
                </CardContent>
