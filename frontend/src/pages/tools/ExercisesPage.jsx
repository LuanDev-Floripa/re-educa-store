...
const ExercisesPage = () => {
  ...
  useEffect(() => {
    const loadExercises = async () => {
      try {
        ...
        if (response.exercises || response.data || Array.isArray(response)) {
          const exercisesList = response.exercises || response.data || response;
  ...
  useEffect(() => {
    filterExercises();
  }, [searchTerm, selectedCategory, selectedDifficulty, selectedMuscleGroup, selectedEquipment, activeTab, exercises]);
  ...
  const filterExercises = () => {
    ...
    if (activeTab === 'favorites') {
      // Filtrar favoritos
      const favoriteIds = new Set(favoriteExercises.map(e => e.id || e.exercise_id));
      filtered = filtered.filter(exercise => favoriteIds.has(exercise.id));
    } else if (activeTab === 'recent') {
      // Filtrar exercícios recentes (últimos 7 dias)
      const recentIds = new Set(recentExercises.map(e => e.id || e.exercise_id));
      filtered = filtered.filter(exercise => recentIds.has(exercise.id))
        .sort((a, b) => {
          const aDate = recentExercises.find(e => (e.id || e.exercise_id) === a.id)?.created_at || '';
          const bDate = recentExercises.find(e => (e.id || e.exercise_id) === b.id)?.created_at || '';
  ...
  const handleAddToFavorite = (exercise) => {
    const isFavorite = favoriteExercises.some(e => (e.id || e.exercise_id) === exercise.id);
    
    if (isFavorite) {
      const updated = favoriteExercises.filter(e => (e.id || e.exercise_id) !== exercise.id);
      setFavoriteExercises(updated);
      localStorage.setItem('exercise_favorites', JSON.stringify(updated));
      toast.success('Removido dos favoritos');
    } else {
      const updated = [...favoriteExercises, { 
        id: exercise.id, 
  ...
  const handleAddToWorkout = (exercise) => {
    ...
    const recent = recentExercises.filter(e => (e.id || e.exercise_id) !== exercise.id);
    const updated = [
      { 
        id: exercise.id, 
        exercise_id: exercise.id,
  ...
  const getStats = () => {
    ...
    const noEquipmentCount = exercises.filter(e => e.equipment.includes('Nenhum')).length;
    ...
    <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
            ...
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-purple-600">{stats.noEquipment}</div>
                <div className="text-sm text-gray-600 dark:text-gray-400">Sem Equipamento</div>
              </CardContent>
            </Card>
          </div>
        </div>
