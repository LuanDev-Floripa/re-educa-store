...
const imcSchema = z.object({
  weight: z
    .number()
    .min(20, "Peso deve ser pelo menos 20kg")
    .max(300, "Peso deve ser no máximo 300kg"),
  height: z
    .number()
    .min(0.5, "Altura deve ser pelo menos 0.5m")
    .max(3, "Altura deve ser no máximo 3m"),
});

/**
 * IMCCalculatorPage
 * Calcula IMC com validação, histórico e integração com API (com guards).
 * @returns {JSX.Element}
 */
export const IMCCalculatorPage = () => {
  ...
  React.useEffect(() => {
    if (
      ...
      const imc = calculateIMC(Number(watchedWeight), Number(watchedHeight));
      if (imc) {
  ...
  const loadHistory = async () => {
    try {
      ...
      if (!apiService?.health?.getIMCHistory) {
  ...
  const onSubmit = async (data) => {
    try {
      const imc = calculateIMC(Number(data.weight), Number(data.height));
      const classification = classifyIMC(imc);

      if (typeof request !== "function") {
        throw new Error("Serviço de rede indisponível");
      }
      if (!apiService?.health?.calculateIMC) {
        throw new Error("Serviço de cálculo indisponível");
      }
      await request(() =>
      ...
      toast.error(e?.message || "Erro ao salvar IMC. Tente novamente.");
    }
  };
  ...
  const getImcColor = (imc) => {
    ...
    if (imc < 30) return "text-primary";
    return "text-destructive";
  };

  const getImcBgColor = (imc) => {
    if (imc < 18.5) return "bg-primary/10 border-primary/20";
    if (imc < 25) return "bg-primary/10 border-primary/20";
    if (imc < 30) return "bg-primary/10 border-primary/20";
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        ...
        <Button
          onClick={() => {
            // Contexto da ferramenta será definido pelo UnifiedAIAssistant global
          }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div>
                  ...
                  {watchedWeight && (
                    <p className="text-xs text-muted-foreground mt-1">
                      {formatWeight(Number(watchedWeight) || 0)}
                    </p>
                  )}
                </div>
                <div>
                  ...
                  {watchedHeight && (
                    <p className="text-xs text-muted-foreground mt-1">
                      {formatHeight(Number(watchedHeight) || 0)}
                    </p>
                  )}
                </div>
              </div>
        ...
        <div className="space-y-6">
          {imcResult && (
            <Card
              className={`shadow-lg border-2 ${getImcBgColor(parseFloat(imcResult.imc))}`}
            >
              <CardHeader>
              ...
              <CardContent>
                <div className="text-center">
                  <div
                    className={`text-6xl font-bold ${getImcColor(parseFloat(imcResult.imc))} mb-4`}
                  >
                    {imcResult.imc}
                  </div>
                  <p className="text-muted-foreground">
                    {imcResult.classification === "Abaixo do peso" &&
                      "Considere consultar um nutricionista para ganhar peso de forma saudável."}
                    {imcResult.classification === "Peso normal" &&
                      "Parabéns! Mantenha seus hábitos saudáveis."}
                    {imcResult.classification === "Sobrepeso" &&
                      "Considere ajustar sua alimentação e aumentar a atividade física."}
                    {imcResult.classification === "Obesidade" &&
                      "Recomendamos consultar um profissional de saúde para um plano personalizado."}
                  </p>
                </div>
              </CardContent>
            </Card>
                      ...
                      >
                        <div>
                          <p className="font-medium">
                            {entry.imc} - {entry.classification}
                          </p>
      ...
      <div className="mt-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          <Card className="text-center p-6">
            ...
            <p className="text-sm text-muted-foreground">
              O Índice de Massa Corporal é uma medida que relaciona peso e
              altura para avaliar se o peso está adequado.
            </p>
          </Card>
          <Card className="text-center p-6">
            <div className="text-primary text-4xl mb-2">🎯</div>
            ...
            <p className="text-sm text-muted-foreground">
              IMC entre 18.5 e 24.9 é considerado peso normal. Valores abaixo ou
              acima indicam necessidade de atenção.
            </p>
          </Card>
          <Card className="text-center p-6">
            ...
            <p className="text-sm text-muted-foreground">
              O IMC não considera massa muscular, idade ou distribuição de
              gordura. Consulte sempre um profissional.
            </p>
          </Card>
          <Card className="text-center p-6">
            <div className="text-primary text-4xl mb-2">💡</div>
            ...
            <p className="text-sm text-muted-foreground">
              Use o IMC como uma ferramenta de acompanhamento, não como
              diagnóstico definitivo de saúde.
            </p>
          </Card>
        </div>
      </div>
