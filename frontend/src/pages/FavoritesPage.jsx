...
const FavoritesPage = () => {
  ...
  const productFavorites = getFavoritesByType('product');
  const exerciseFavorites = getFavoritesByType('exercise');
  const workoutPlanFavorites = getFavoritesByType('workout_plan');
  const toolFavorites = getFavoritesByType('tool');
  ...
  const getItemTypeLabel = (type) => {
    switch (type) {
      ...
      case 'workout_plan': return 'Plano de Treino';
  ...
  const getItemUrl = (item) => {
    switch (item.type) {
      case 'product': return `/store/product/${item.id}`;
      case 'exercise': return `/tools/exercises`;
      case 'workout_plan': return `/tools/workout-plans`;
      ...
      switch (type) {
        case 'products':
          return {
            ...
            description: 'Adicione produtos aos seus favoritos para encontrá-los facilmente'
          };
        case 'exercises':
          return {
            title: 'Nenhum exercício favorito',
            description: 'Marque exercícios como favoritos para acessá-los rapidamente'
          };
        case 'workout_plans':
          return {
    ...
    <div className="max-w-7xl mx-auto p-6">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
            ...
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-green-600">{getFavoritesCountByType('product')}</div>
                <div className="text-sm text-gray-600 dark:text-gray-400">Produtos</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-purple-600">{getFavoritesCountByType('exercise')}</div>
                <div className="text-sm text-gray-600 dark:text-gray-400">Exercícios</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-orange-600">{getFavoritesCountByType('workout_plan')}</div>
                <div className="text-sm text-gray-600 dark:text-gray-400">Planos</div>
              </CardContent>
            </Card>
            <Card>
              <CardContent className="p-4 text-center">
                <div className="text-2xl font-bold text-cyan-600">{getFavoritesCountByType('tool')}</div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-6">
          <TabsList className="grid w-full grid-cols-5">
            ...
            <TabsTrigger value="workout_plans" className="flex items-center space-x-2">
