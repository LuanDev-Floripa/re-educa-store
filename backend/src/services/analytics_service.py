...
class AnalyticsService:
    ...
    def get_users_analytics(self, period: str = 'month') -> Dict[str, Any]:
        ...
        try:
            ...
            active_users = (
                len(set(act.get('user_id') for act in activities_result.data))
                if activities_result.data else 0
            )
            ...
            users_by_day = self._group_users_by_day(
                new_users_result.data if new_users_result.data else [],
                start_date, end_date
            )
    ...
    def get_products_analytics(self, period: str = 'month') -> Dict[str, Any]:
        ...
        try:
            ...
            if order_ids:
                items_result = (
                    self.db._make_request("GET", 'order_items')
                    .select('*')
            ...
            for product_id, sales in sorted_products:
                product = (
                    next(
                        (p for p in products_result.data if p['id'] == product_id),
                        None
                    ) if products_result.data else None
                )
                if product:
                    top_products.append({
                        'id': product_id,
            ...
            category_sales = self._group_sales_by_category(
                order_items,
                products_result.data if products_result.data else []
            )
            ...
            ]) if products_result.data else 0
            out_of_stock = len([
                p for p in products_result.data
    ...
    def _get_top_products_sold(self, start_date: str, end_date: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for product_id, totals in sorted_products:
                product_result = (
                    self.db._make_request("GET", 'products')
        ...
        try:
            ...
            if previous_orders.data:
                previous_revenue = sum(
                    o.get('total', 0)
                    for o in previous_orders.data
                    if o.get('status') in ['paid', 'completed']
                )
                previous_orders_count = len([
                    o for o in previous_orders.data
                    if o.get('status') in ['paid', 'completed']
                ])
            ...
            if current_orders.data:
                current_revenue = sum(
                    o.get('total', 0)
                    for o in current_orders.data
                    if o.get('status') in ['paid', 'completed']
                )
                current_orders_count = len([
                    o for o in current_orders.data
                    if o.get('status') in ['paid', 'completed']
                ])
            ...
            orders_change = (
                ...
                if previous_orders_count > 0 else 0
            )
