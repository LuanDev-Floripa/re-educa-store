...
class TwoFactorService:
    ...
    def generate_qr_code(self, user_email: str, secret_key: str) -> str:
        ...
        try:
            ...
            totp_uri = pyotp.totp.TOTP(secret_key).provisioning_uri(name=user_email, issuer_name="RE-EDUCA Store")
    ...
    def setup_two_factor(self, user_id: str, user_email: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.repo.upsert_config(
                user_id=user_id, secret_key=secret_key, backup_codes=",".join(backup_codes), enabled=False
            )
            ...
            logger.error(f"Erro ao configurar 2FA: {str(e)}", exc_info=True)
    ...
    def verify_totp_code(self, user_id: str, totp_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao verificar código TOTP: {str(e)}", exc_info=True)
            return {"success": False, "error": "Erro interno do servidor"}
    ...
    def verify_backup_code(self, user_id: str, backup_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao verificar código de backup: {str(e)}", exc_info=True)
            return {"success": False, "error": "Erro interno do servidor"}
    ...
    def enable_two_factor(self, user_id: str, totp_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.user_repo.update(user_id, {"two_factor_enabled": True, "updated_at": datetime.now().isoformat()})
            ...
            logger.error(f"Erro ao habilitar 2FA: {str(e)}", exc_info=True)
            return {"success": False, "error": "Erro interno do servidor"}
    ...
    def disable_two_factor(self, user_id: str, totp_code: str = None, backup_code: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Atualiza tabela users
            self.user_repo.update(user_id, {"two_factor_enabled": False, "updated_at": datetime.now().isoformat()})
            ...
            logger.error(f"Erro ao desabilitar 2FA: {str(e)}", exc_info=True)
            return {"success": False, "error": "Erro interno do servidor"}
    ...
    def is_two_factor_enabled(self, user_id: str) -> bool:
        ...
        try:
            ...
            logger.error(f"Erro ao verificar status do 2FA: {str(e)}", exc_info=True)
            return False
    ...
    def get_backup_codes(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "backup_codes": backup_codes, "remaining": len(backup_codes)}

        except (ValueError, KeyError) as e:
            logger.warning(f"Erro de validação: {str(e)}")
            # Tratamento específico pode ser adicionado aqui
        except Exception as e:
            logger.error(f"Erro ao buscar códigos de backup: {str(e)}", exc_info=True)
            return {"success": False, "error": "Erro interno do servidor"}
    ...
    def regenerate_backup_codes(self, user_id: str, totp_code: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao regenerar códigos de backup: {str(e)}", exc_info=True)
            return {"success": False, "error": "Erro interno do servidor"}
