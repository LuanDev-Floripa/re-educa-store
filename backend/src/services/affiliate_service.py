...
class AffiliateService(BaseService):
    ...
    Utiliza AffiliateRepository para acesso padronizado aos dados.
    ...
    def get_hotmart_token(self) -> Optional[str]:
        ...
        try:
            ...
            logger.error(f"Erro ao obter token Hotmart: {str(e)}", exc_info=True)
            return None
    ...
    def get_hotmart_products(self, page: int = 0, size: int = 20) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao buscar produtos Hotmart: {str(e)}", exc_info=True)
    ...
    def track_hotmart_sale(self, sale_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao registrar venda Hotmart: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}
    ...
    def get_kiwify_products(self, page: int = 1, limit: int = 20) -> Dict[str, Any]:
        ...
        try:
            ...
            headers = {"Authorization": f"Bearer {self.kiwify_api_key}", "Content-Type": "application/json"}
            ...
            logger.error(f"Erro ao buscar produtos Kiwify: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}
    ...
    def track_kiwify_sale(self, sale_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao registrar venda Kiwify: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}
    ...
    def verify_hotmart_webhook(self, signature: str, payload: bytes) -> bool:
        ...
        try:
            ...
            expected_signature = hmac.new(
                self.hotmart_webhook_secret.encode("utf-8"), payload, hashlib.sha256
            ).hexdigest()
            ...
            logger.error(f"Erro ao verificar webhook Hotmart: {str(e)}", exc_info=True)
            return False
    ...
    def verify_kiwify_webhook(self, signature: str, payload: bytes) -> bool:
        ...
        try:
            ...
            expected_signature = hmac.new(
                self.kiwify_webhook_secret.encode("utf-8"), payload, hashlib.sha256
            ).hexdigest()
            ...
            logger.error(f"Erro ao verificar webhook Kiwify: {str(e)}", exc_info=True)
            return False
    ...
    def get_logs_products(self, page: int = 1, limit: int = 20) -> Dict[str, Any]:
        ...
        try:
            ...
            headers = {"Authorization": f"Bearer {self.logs_api_key}", "Content-Type": "application/json"}
            ...
            logger.error(f"Erro ao buscar produtos Logs: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}
    ...
    def get_braip_products(self, page: int = 1, limit: int = 20) -> Dict[str, Any]:
        ...
        try:
            ...
            headers = {"Authorization": f"Bearer {self.braip_api_key}", "Content-Type": "application/json"}
            ...
            logger.error(f"Erro ao buscar produtos Braip: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}
        ...
        try:
            ...
            for platform in platforms:
                try:
                    ...
                    logger.error(f"Erro ao sincronizar {platform}: {str(e)}", exc_info=True)
                    continue
            ...
            logger.error(f"Erro ao sincronizar produtos: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}

    def get_affiliate_products(
        self, platform: str = None, category: str = None, page: int = 1, limit: int = 20
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "products": paginated_products, "page": page, "limit": limit}

        except (ValueError, KeyError) as e:
            logger.warning(f"Erro de validação: {str(e)}")
            # Tratamento específico pode ser adicionado aqui
        except Exception as e:
            self.logger.error(f"Erro ao buscar produtos afiliados: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}

    def get_affiliate_sales(
        self, platform: str = None, start_date: str = None, end_date: str = None, page: int = 1, limit: int = 20
    ) -> Dict[str, Any]:
        """Busca vendas afiliadas"""
        try:
            sales = self.repo.find_all_filtered(
                platform=platform, start_date=start_date, end_date=end_date, limit=limit, offset=(page - 1) * limit
            )

            total = self.repo.count_filtered(platform=platform, start_date=start_date, end_date=end_date)
            ...
            logger.error(f"Erro ao buscar vendas afiliadas: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}

    def calculate_commission(self, platform: str, amount: float) -> float:
        """Calcula comissão baseada na plataforma"""
        commission_rates = {"hotmart": 0.30, "kiwify": 0.25, "logs": 0.35, "braip": 0.40}  # 30%  # 25%  # 35%  # 40%
