...
class HealthCalculatorService:
    ...
    def calculate_bmi(self, height_cm: float, weight_kg: float) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao calcular IMC: {e}")
            return {'error': 'Erro interno do servidor'}
    
    def calculate_calories(self, age: int, gender: str, height_cm: float, 
                          weight_kg: float, activity_level: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao calcular calorias: {e}")
            return {'error': 'Erro interno do servidor'}
    
    def calculate_hydration(self, weight_kg: float, activity_level: str, 
                           climate: str = 'temperate') -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao calcular hidratação: {e}")
            return {'error': 'Erro interno do servidor'}
    
    def calculate_body_fat(self, age: int, gender: str, height_cm: float, 
                          weight_kg: float, waist_cm: float, neck_cm: float) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Erro ao calcular gordura corporal: {e}")
            return {'error': 'Erro interno do servidor'}
    
    def save_calculation(self, user_id: str, calculation_type: str, 
                        input_data: Dict[str, Any], result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Insere no Supabase usando método helper
            response = self.supabase.insert_health_calculation(calculation_data)
            ...
            if response:
                ...
                if calculation_type == 'bmi':
                    ...
                    try:
                        self.supabase.table('imc_history').insert(imc_data).execute()
                        logger.debug(f"Histórico de IMC salvo para usuário {user_id}")
                    except Exception as e:
                        logger.warning(f"Tabela imc_history pode não existir: {e}")
                    ...
                    try:
                        self.supabase.table('calories_history').insert(calories_data).execute()
                        logger.debug(f"Histórico de calorias salvo para usuário {user_id}")
                    except Exception as e:
                        logger.warning(f"Tabela calories_history pode não existir: {e}")
                    ...
                    try:
                        self.supabase.table('hydration_history').insert(hydration_data).execute()
                        logger.debug(f"Histórico de hidratação salvo para usuário {user_id}")
                    except Exception as e:
                        logger.warning(f"Tabela hydration_history pode não existir: {e}")
