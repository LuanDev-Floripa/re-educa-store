...
class PredictiveAnalysisService:
    ...
    def predict_health_metrics(self, user_id: str, days_ahead: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            if not health_data or len(health_data) < 10:
                return {"success": False, "error": "Dados insuficientes para predição (mínimo 10 registros)"}
            ...
            if "imc_value" in df.columns:
                imc_prediction = self._predict_metric(df, "imc_value", "date", days_ahead)
                predictions["imc"] = imc_prediction

            # Prediz peso
            if "weight" in df.columns:
                weight_prediction = self._predict_metric(df, "weight", "date", days_ahead)
            ...
            logger.error(f"Erro na predição de métricas de saúde: {str(e)}", exc_info=True)
    ...
    def predict_user_behavior(self, user_id: str, behavior_type: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not behavior_data:
                return {"success": False, "error": "Dados comportamentais insuficientes"}
            ...
            if behavior_type == "purchases":
                ...
                return {"success": False, "error": f"Tipo de comportamento não suportado: {behavior_type}"}

        except (ValueError, KeyError) as e:
            logger.warning(f"Erro de validação: {str(e)}")
            # Tratamento específico pode ser adicionado aqui
        except Exception as e:
            logger.error(f"Erro na predição de comportamento: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}
    ...
    def predict_churn_risk(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not engagement_metrics:
                return {"success": False, "error": "Dados de engajamento insuficientes"}
            ...
            if churn_score >= 0.7:
                ...
                risk_level = "Médio"
                recommendations = ["Enviar lembretes de uso", "Oferecer novos recursos", "Solicitar feedback"]
            else:
                risk_level = "Baixo"
                recommendations = ["Manter engajamento atual", "Oferecer recursos premium", "Solicitar indicações"]
            ...
            logger.error(f"Erro na predição de churn: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}
    ...
    def predict_optimal_interventions(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not user_patterns:
                return {"success": False, "error": "Padrões do usuário não identificados"}
            ...
            logger.error(f"Erro na predição de intervenções: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}
    ...
    def predict_seasonal_trends(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if not seasonal_data:
                return {"success": False, "error": "Dados sazonais insuficientes"}
            ...
            logger.error(f"Erro na predição de tendências sazonais: {str(e)}", exc_info=True)
            return {"success": False, "error": str(e)}
    ...
    def _get_user_health_history(self, user_id: str) -> List[Dict]:
        ...
        Utiliza PredictiveAnalysisRepository para acesso padronizado aos dados.
        """
        try:
                    ...
                    health_data.append(
                        {
                            ...
                            "user_id": user_id,
                            "activity_level": (
                                min(10, record.get("duration_minutes", 0) / 6) if record.get("duration_minutes") else 0
                            ),
                        }
                    )
            ...
            logger.error(f"Erro ao buscar histórico de saúde: {str(e)}", exc_info=True)
            return []
    ...
    def _predict_metric(self, df: pd.DataFrame, metric: str, date_col: str, days_ahead: int) -> Dict:
        ...
        try:
            ...
            logger.error(f"Erro na predição de métrica: {str(e)}", exc_info=True)
            return {"error": str(e)}
        ...
        try:
            ...
            if user_id:
                ...
                if exercise_response_data and len(exercise_response_data) > 0:
                    ...
                    avg_duration = (
                        exercise_df["duration_minutes"].mean() if "duration_minutes" in exercise_df.columns else 0
                    )
                    ...
                    if len(exercise_df) >= 7:
                        ...
                        recent_avg_duration = (
                            recent_days["duration_minutes"].mean() if "duration_minutes" in recent_days.columns else 0
                        )

                        if len(older_days) > 0:
                            older_freq = len(older_days["completed_at"].dt.date.unique()) / (len(older_days) / 7)
                            older_avg_duration = (
                                older_days["duration_minutes"].mean() if "duration_minutes" in older_days.columns else 0
                            )

                            trend = (
                                "increasing"
                                if (recent_freq > older_freq or recent_avg_duration > older_avg_duration)
                                else (
                                    "stable"
                                    if (recent_freq == older_freq and recent_avg_duration == older_avg_duration)
                                    else "decreasing"
                                )
                            )
                        else:
                    ...
                    return {
                        ...
                        "recommendation": (
                            "Mantenha a atividade atual"
                            if activity_score >= 7
                            else (
                                "Aumente a frequência de exercícios"
                                if activity_score < 4
                                else "Melhore gradualmente a consistência"
                            )
                        ),
                    }
            ...
            logger.error(f"Erro na predição de atividade: {str(e)}", exc_info=True)
            return {"error": str(e)}
    ...
    def _predict_health_risk(self, df: pd.DataFrame, predictions: Dict) -> Dict:
        ...
        try:
            ...
            logger.error(f"Erro na predição de risco: {str(e)}", exc_info=True)
            return {"error": str(e)}
    ...
    def _calculate_prediction_confidence(self, df: pd.DataFrame) -> float:
        ...
        try:
            ...
            logger.error(f"Erro no cálculo de confiança: {str(e)}", exc_info=True)
            return 50.0
    ...
    def _get_user_behavior_data(self, user_id: str, behavior_type: str) -> List[Dict]:
        ...
        try:
            ...
            logger.error(f"Erro ao buscar dados comportamentais: {str(e)}", exc_info=True)
            return []
    ...
    def _predict_purchase_behavior(self, df: pd.DataFrame, user_id: str) -> Dict:
        ...
        try:
            ...
            logger.error(f"Erro na predição de compras: {str(e)}", exc_info=True)
            return {"error": str(e)}
    ...
    def _predict_exercise_behavior(self, df: pd.DataFrame, user_id: str) -> Dict:
        ...
        try:
            ...
            logger.error(f"Erro na predição de exercícios: {str(e)}", exc_info=True)
            return {"error": str(e)}
        ...
        try:
            ...
            return {
                ...
                "nutrition_consistency": (
                    "high" if avg_meals_per_day >= 3 else "medium" if avg_meals_per_day >= 2 else "low"
                ),
            ...
            logger.error(f"Erro na predição nutricional: {str(e)}", exc_info=True)
            return {"error": str(e)}
    ...
    def _get_engagement_metrics(self, user_id: str) -> Dict:
        ...
        try:
            ...
            logger.error(f"Erro ao coletar métricas de engajamento: {str(e)}", exc_info=True)
            return {}
    ...
    def _calculate_churn_score(self, metrics: Dict) -> float:
        ...
        try:
            ...
            logger.error(f"Erro no cálculo de churn score: {str(e)}", exc_info=True)
            return 0.5
    ...
    def _analyze_user_patterns(self, user_id: str) -> Dict:
        ...
        try:
            ...
            logger.error(f"Erro ao analisar padrões: {str(e)}", exc_info=True)
            return {}
            ...
            for record in imc_data:
                ...
                if date_str:
                    try:
                        ...
                        logger.debug(f"Erro ao processar dados de saúde (não crítico): {str(e)}")
                        pass
                    except Exception as e:
                        logger.warning(f"Erro inesperado ao processar dados de saúde: {str(e)}")
                        pass
                            ...
                            all_data.append(
                                {
                                    ...
                                    "season": season,
                                    "activity_level": (
                                        min(10, record.get("duration_minutes", 0) / 6)
    ...
    def _get_season_from_date(self, date: datetime) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(f"Erro ao determinar estação (esperado em alguns casos): {str(e)}")
            return None
        except (ValueError, KeyError) as e:
            logger.warning(f"Erro de validação: {str(e)}")
            # Tratamento específico pode ser adicionado aqui
        except Exception as e:
            logger.warning(f"Erro inesperado ao determinar estação: {str(e)}")
            return None
    ...
    def _predict_seasonal_changes(self, trends: Dict, current_season: str, next_season: str) -> Dict:
        ...
        try:
            ...
            logger.error(f"Erro ao predizer mudanças sazonais: {str(e)}", exc_info=True)
            return {}
    ...
    def _get_risk_recommendations(self, risk_level: str, risk_factors: int) -> List[str]:
        ...
        if risk_level == "Alto":
            ...
            return ["Mantenha monitoramento regular", "Foque em melhorias graduais", "Acompanhe progresso semanalmente"]
        else:
            return ["Continue com hábitos saudáveis", "Mantenha rotina atual", "Defina novos desafios"]
