...
Implementa resiliência HTTP integrada para requisições robustas.
"""
import os
...
class SupabaseClient:
    ...
    def __init__(self):
        ...
        # Evitar hardcode de chaves sensíveis em fallback
        ...
        # Cliente HTTP resiliente para Supabase
        try:
            from utils.http_resilience import ResilientHTTPClient
            ...
            # Fallback para evitar quebra caso importação falhe
    ...
    def _make_request(self, method: str, endpoint: str, data: Optional[Dict] = None) -> Dict:
        ...
        """
        try:
            # Usar cliente resiliente se disponível, senão usar requests direto
            if self.http_client:
                if method.upper() == "GET":
                    response = self.http_client.get(endpoint, headers=self.headers, params=data)
                elif method.upper() == "POST":
                    response = self.http_client.post(endpoint, headers=self.headers, json=data)
                elif method.upper() == "PUT":
                    response = self.http_client.put(endpoint, headers=self.headers, json=data)
                elif method.upper() == "PATCH":
                    response = self.http_client.patch(endpoint, headers=self.headers, json=data)
                elif method.upper() == "DELETE":
                    response = self.http_client.delete(endpoint, headers=self.headers)
                else:
                    raise ValueError(f"Método HTTP não suportado: {method}")
            else:
                # Fallback para requests direto com timeout
                url = f"{self.url}/rest/v1/{endpoint}"
                if method.upper() == "GET":
                    response = requests.get(url, headers=self.headers, params=data, timeout=15)
                elif method.upper() == "POST":
