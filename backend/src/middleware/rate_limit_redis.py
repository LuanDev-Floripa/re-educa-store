...
def get_limiter_key():
    ...
    # Excluir endpoints de health check do rate limiting
    if request.path in ["/health", "/health/detailed"]:
        # Retornar chave especial que será ignorada
        return "health-check"
...
def setup_rate_limiting_redis(app: Flask):
    ...
    try:
        ...
        if redis_password:
            ...
            test_client = redis.Redis(host=redis_host, port=redis_port, decode_responses=True, socket_connect_timeout=2)
        ...
        # Redis não disponível - usar fallback para memória
        redis_available = False
        logger.warning(f"Redis não disponível para rate limiting: {e}. Usando fallback em memória.")

    # Inicializar Flask-Limiter com storage apropriado
    ...
    def exempt_health_endpoints():
        if request.path in ["/health", "/health/detailed"]:
            # Marcar para não aplicar rate limiting
            from flask import g
...
def get_limiter() -> Limiter:
    ...
    """
    if _limiter is None:
        raise RuntimeError("Rate limiter não foi inicializado. " "Chame setup_rate_limiting_redis() primeiro.")
    return _limiter
