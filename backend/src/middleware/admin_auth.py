...
def admin_required(f):
    ...
    def decorated_function(*args, **kwargs):
        try:
            ...
            logger.error(f"Erro na verificação de admin: {str(e)}", exc_info=True)
    ...
    def decorated_function(*args, **kwargs):
        try:
            ...
            if user.get("role") != "admin" and user.get("id") != user_id:
                logger.warning(
                    f"Tentativa de acesso não autorizado: {user.get('email')} "
                    f"tentando acessar dados do usuário {user_id}"
                )
            ...
            logger.error(f"Erro na verificação de acesso: {str(e)}", exc_info=True)
            return jsonify({"error": "Erro interno do servidor"}), 500
