...
class TestSecurity:
    ...
    def test_sql_injection_protection(self, client):
        ...
        response = client.post(
            "/api/auth/login",
            data=json.dumps({"email": malicious_payload, "password": "password"}),
        ...
        assert (
            "database" not in error_msg or "error" not in error_msg
    ...
    def test_xss_protection(self, client):
        ...
        response = client.post(
            "/api/auth/register",
            data=json.dumps({"name": xss_payload, "email": "test@example.com", "password": "password123"}),
            content_type="application/json",
        )
    ...
    def test_input_validation(self, client):
        ...
        response = client.post(
            "/api/auth/register",
            data=json.dumps({"name": "Test User", "email": "invalid-email", "password": "password123"}),
            content_type="application/json",
        )
        ...
        response = client.post(
            "/api/auth/register",
            data=json.dumps({"name": "Test User", "email": "test@example.com", "password": "123"}),
            content_type="application/json",
        )
    ...
    def test_file_upload_security(self, client):
        ...
        response = client.post(
            "/api/upload", data={"file": (malicious_file, "malicious.php")}, content_type="multipart/form-data"
        )
    ...
    def test_cors_configuration(self, client):
        ...
        response = client.get("/api/auth/me", headers={"Origin": "https://malicious-site.com"})
    ...
    def test_logging_security(self, client):
        ...
        with patch("src.middleware.logging.logger") as mock_logger:
            ...
            _ = client.post(
                "/api/auth/login",
                data=json.dumps({"email": "test@example.com", "password": "sensitive_password"}),
                content_type="application/json",
            )
    ...
    def test_file_type_validation(self, client):
        ...
        response = client.post(
            "/api/upload", data={"file": (dangerous_file, "malicious.exe")}, content_type="multipart/form-data"
        )
    ...
    def test_content_length_limit(self, client):
        ...
        response = client.post(
            "/api/upload", data={"file": (large_payload, "large.txt")}, content_type="multipart/form-data"
        )
