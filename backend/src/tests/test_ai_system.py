...
class TestAISystem:
    ...
    def test_ai_chat_success(self, client, auth_headers, mock_supabase):
        ...
        """
        with patch("main.supabase", mock_supabase), patch("main.request") as mock_request:
            ...
            chat_data = {"message": "Como posso melhorar minha alimentação?", "context": "nutrition"}

            response = client.post(
                "/api/ai/chat", data=json.dumps(chat_data), content_type="application/json", headers=auth_headers
            )
    ...
    def test_ai_chat_empty_message(self, client, auth_headers):
        ...
        chat_data = {"message": "", "context": "nutrition"}
    ...
    def test_ai_chat_long_message(self, client, auth_headers, mock_supabase):
        ...
        with patch("main.supabase", mock_supabase), patch("main.request") as mock_request:
            ...
            long_message = "A" * 1000
            chat_data = {"message": long_message, "context": "general"}
    ...
    def test_ai_chat_different_contexts(self, client, auth_headers, mock_supabase):
        ...
        with patch("main.supabase", mock_supabase), patch("main.request") as mock_request:
            ...
            for context in contexts:
                chat_data = {"message": f"Teste para contexto {context}", "context": context}
    ...
    def test_get_chat_history_with_limit(self, client, auth_headers, mock_supabase):
        ...
        with patch("main.supabase", mock_supabase), patch("main.request") as mock_request:
            ...
            response = client.get("/api/ai/history?limit=1", headers=auth_headers)
    ...
    def test_get_chat_history_with_context_filter(self, client, auth_headers, mock_supabase):
        ...
        with patch("main.supabase", mock_supabase), patch("main.request") as mock_request:
            ...
            response = client.get("/api/ai/history?context=nutrition", headers=auth_headers)
    ...
    def test_ai_chat_with_metadata(self, client, auth_headers, mock_supabase):
        ...
        with patch("main.supabase", mock_supabase), patch("main.request") as mock_request:
            ...
            chat_data = {
                ...
                "context": "general",
                "metadata": {"user_preferences": "vegetarian", "health_conditions": "diabetes", "language": "pt-BR"},
            }
    ...
    def test_ai_chat_error_handling(self, client, auth_headers, mock_supabase):
        ...
        with patch("main.supabase", mock_supabase), patch("main.request") as mock_request:
            ...
            chat_data = {"message": "Teste de erro", "context": "general"}
    ...
    def test_ai_chat_rate_limiting(self, client, auth_headers, mock_supabase):
        ...
        with patch("main.supabase", mock_supabase), patch("main.request") as mock_request:
            ...
            for i in range(5):
                chat_data = {"message": f"Mensagem {i}", "context": "general"}
    ...
    def test_ai_chat_context_validation(self):
        ...
        for context in valid_contexts:
            assert self.is_valid_context(context) is True

        invalid_contexts = ["invalid", "unknown", "", None]
        for context in invalid_contexts:
            assert self.is_valid_context(context) is False

    @pytest.mark.ai
    def test_ai_chat_message_validation(self):
        """Testa validação de mensagens de chat"""
        # Mensagens válidas
        assert self.is_valid_message("Olá, como posso ajudar?") is True
        assert self.is_valid_message("Teste") is True
        assert self.is_valid_message("A" * 100) is True

        # Mensagens inválidas
        assert self.is_valid_message("") is False
        assert self.is_valid_message(None) is False
        assert self.is_valid_message("A" * 10001) is False  # Muito longa
