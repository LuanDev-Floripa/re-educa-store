...
def calculate_imc(weight: float, height: float) -> Dict[str, Any]:
    ...
    return {
        ...
        "weight_range": {"min": round(min_ideal, 1), "max": round(max_ideal, 1)},
    }
...
def calculate_calories(age: int, weight: float, height: float, gender: str, activity_level: str) -> Dict[str, Any]:
    ...
    return {"bmr": round(bmr), "daily_calories": round(daily_calories), "activity_multiplier": multiplier}
...
def calculate_macros(calories: float, protein_percent: float = 20, fat_percent: float = 30) -> Dict[str, Any]:
    ...
    return {
        "protein": {"grams": round(protein_grams, 1), "calories": round(protein_cals), "percentage": protein_percent},
        "carbs": {"grams": round(carbs_grams, 1), "calories": round(carbs_cals), "percentage": carbs_percent},
        "fat": {"grams": round(fat_grams, 1), "calories": round(fat_cals), "percentage": fat_percent},
    }
