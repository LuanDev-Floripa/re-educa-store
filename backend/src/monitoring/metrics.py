...
class MetricsCollector:
    ...
    def __init__(self, registry: Optional[Any] = None):
        ...
        self.http_requests_total = Counter(
            "http_requests_total", "Total de requisições HTTP", ["method", "endpoint", "status"], registry=self.registry
        )
        ...
        self.db_queries_total = Counter(
            "db_queries_total", "Total de queries no banco de dados", ["operation", "table"], registry=self.registry
        )
        ...
        self.cache_hits_total = Counter(
            "cache_hits_total", "Total de hits no cache", ["cache_type"], registry=self.registry
        )

        self.cache_misses_total = Counter(
            "cache_misses_total", "Total de misses no cache", ["cache_type"], registry=self.registry
        )

        self.cache_size_bytes = Gauge(
            "cache_size_bytes", "Tamanho do cache em bytes", ["cache_type"], registry=self.registry
        )

        # Métricas de filas
        self.queue_tasks_total = Counter(
            "queue_tasks_total", "Total de tarefas processadas", ["queue_name", "status"], registry=self.registry
        )

        self.queue_tasks_in_queue = Gauge(
            "queue_tasks_in_queue", "Número de tarefas na fila", ["queue_name", "priority"], registry=self.registry
        )
        ...
        self.active_users = Gauge("active_users", "Número de usuários ativos", registry=self.registry)

        self.user_registrations_total = Counter(
            "user_registrations_total", "Total de registros de usuários", registry=self.registry
        )

        self.user_logins_total = Counter(
            "user_logins_total", "Total de logins de usuários", ["status"], registry=self.registry
        )

        # Métricas de saúde
        self.health_checks_total = Counter(
            "health_checks_total", "Total de verificações de saúde", ["check_type", "status"], registry=self.registry
        )
        ...
        self.ai_requests_total = Counter(
            "ai_requests_total", "Total de requisições para IA", ["ai_type", "status"], registry=self.registry
        )
        ...
        self.payment_amount_total = Counter(
            "payment_amount_total", "Valor total dos pagamentos", ["payment_method", "status"], registry=self.registry
        )

        # Métricas de sistema
        self.system_memory_bytes = Gauge("system_memory_bytes", "Uso de memória do sistema", registry=self.registry)

        self.system_cpu_percent = Gauge("system_cpu_percent", "Uso de CPU do sistema", registry=self.registry)

        self.system_disk_usage_percent = Gauge(
            "system_disk_usage_percent", "Uso de disco do sistema", registry=self.registry
        )
    ...
    def record_http_request(self, method: str, endpoint: str, status: int, duration: float):
        ...
        try:
            ...
            logger.error(f"Erro ao registrar métrica HTTP: {e}", exc_info=True)
    ...
    def record_db_query(self, operation: str, table: str, duration: float):
        ...
        try:
            ...
            logger.error(f"Erro ao registrar métrica de banco: {e}", exc_info=True)
    ...
    def set_queue_tasks_count(self, queue_name: str, priority: str, count: int):
        ...
        try:
            ...
            logger.error(f"Erro ao definir contagem de tarefas: {e}", exc_info=True)
    ...
    def record_queue_task_duration(self, queue_name: str, duration: float):
        ...
        try:
            ...
            logger.error(f"Erro ao registrar duração de tarefa: {e}", exc_info=True)
    ...
    def set_active_users(self, count: int):
        ...
        try:
            ...
            logger.error(f"Erro ao definir usuários ativos: {e}", exc_info=True)
    ...
    def record_user_registration(self):
        ...
        try:
            ...
            logger.error(f"Erro ao registrar registro de usuário: {e}", exc_info=True)
    ...
    def record_user_login(self, status: str):
        ...
        try:
            ...
            logger.error(f"Erro ao registrar login: {e}", exc_info=True)
    ...
    def record_health_check(self, check_type: str, status: str, duration: float):
        ...
        try:
            ...
            logger.error(f"Erro ao registrar verificação de saúde: {e}", exc_info=True)
    ...
    def record_ai_request(self, ai_type: str, status: str, response_time: float):
        ...
        try:
            ...
            logger.error(f"Erro ao registrar requisição de IA: {e}", exc_info=True)
    ...
    def set_system_metrics(self, memory_bytes: int, cpu_percent: float, disk_percent: float):
        ...
        try:
            ...
            logger.error(f"Erro ao definir métricas do sistema: {e}", exc_info=True)
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                metrics_collector.record_http_request(
                    method="GET", endpoint=ep, status=status, duration=duration  # Assumindo GET, pode ser melhorado
                )
...
if __name__ == "__main__":
    ...
    logger.info(r"Métricas geradas:")
    logger.info(metrics)
