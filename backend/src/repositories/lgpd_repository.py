...
class LGPDRepository(BaseRepository):
    ...
    def find_consents_by_user(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        """
        try:
            return self.find_all(filters={"user_id": user_id}, order_by="created_at", desc=True)
    ...
    def find_consent_by_type(self, user_id: str, consent_type: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            result = self.find_all(filters={"user_id": user_id, "consent_type": consent_type}, limit=1)
            return result[0] if result else None
        except (ValueError, KeyError) as e:
            logger.warning(f"Erro de validação: {str(e)}")
            # Tratamento específico pode ser adicionado aqui
        except Exception as e:
            self.logger.error(f"Erro ao buscar consentimento: {str(e)}", exc_info=True)
            return None
    ...
    def create_consent(self, consent_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Erro ao criar consentimento: {str(e)}", exc_info=True)
            return None
    ...
    def update_consent(self, consent_id: str, update_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Erro ao atualizar consentimento: {str(e)}", exc_info=True)
            return None
    ...
    def create_access_log(self, log_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Erro ao criar log de acesso: {str(e)}", exc_info=True)
            return None

    def find_access_logs(
        self, accessed_user_id: str, limit: Optional[int] = None, offset: int = 0
    ) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.error(f"Erro ao buscar logs de acesso: {str(e)}", exc_info=True)
            return []
    ...
    def count_access_logs(self, accessed_user_id: str) -> int:
        ...
        try:
            ...
            self.logger.error(f"Erro ao contar logs: {str(e)}", exc_info=True)
            return 0
    ...
    def create_deletion_record(self, deletion_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Erro ao criar registro de exclusão: {str(e)}", exc_info=True)
            return None
    ...
    def get_table_data_by_user(self, user_id: str, table_name: str) -> List[Dict[str, Any]]:
        ...
        """
        try:
            result = self.db.table(table_name).select("*").eq("user_id", user_id).execute()
            return result.data if result.data else []
        except (ValueError, KeyError) as e:
            logger.warning(f"Erro de validação: {str(e)}")
            # Tratamento específico pode ser adicionado aqui
    ...
    def find_exports_by_user(self, user_id: str) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.warning(f"Erro ao buscar exports: {str(e)}")
            return []

    def create_export(self, export_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Cria registro de exportação de dados.

        Args:
            export_data: Dados da exportação

        Returns:
            Export criado ou None
        """
        try:
            from datetime import datetime
            ...
            if "id" not in export_data:
                export_data["id"] = generate_uuid()
            if "created_at" not in export_data:
                export_data["created_at"] = datetime.now().isoformat()

            result = self.db.table("data_exports").insert(export_data).execute()
            return result.data[0] if result.data and len(result.data) > 0 else None
        except (ValueError, KeyError) as e:
            logger.warning(f"Erro de validação: {str(e)}")
            # Tratamento específico pode ser adicionado aqui
        except Exception as e:
            self.logger.error(f"Erro ao criar export: {str(e)}", exc_info=True)
            return None

    def find_scheduled_exports(self, user_id: str) -> List[Dict[str, Any]]:
        """
        Busca exportações agendadas do usuário.

        Args:
            user_id: ID do usuário

        Returns:
            Lista de exportações agendadas
        """
        try:
            result = (
                self.db.table("scheduled_exports")
                .select("*")
                .eq("user_id", user_id)
            ...
            self.logger.warning(f"Erro ao buscar scheduled exports (tabela pode não existir): {str(e)}")
            return []
