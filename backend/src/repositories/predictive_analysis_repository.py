...
class PredictiveAnalysisRepository(BaseRepository):
    ...
    def get_user_imc_history(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            result = self.db.table("imc_history").select("*").eq("user_id", user_id).order("calculated_at").execute()
    ...
    def get_user_workout_sessions(self, user_id: str, days_back: Optional[int] = None) -> List[Dict[str, Any]]:
        """Busca sessões de treino do usuário."""
        try:
            query = self.db.table("workout_sessions").select("*").eq("user_id", user_id)
    ...
    def get_user_food_diary(self, user_id: str, days_back: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    def get_user_hydration_history(self, user_id: str, days_back: Optional[int] = None) -> List[Dict[str, Any]]:
        """Busca histórico de hidratação do usuário."""
        try:
            query = self.db.table("hydration_history").select("*").eq("user_id", user_id)
    ...
    def get_user_activities(self, user_id: str, days_back: Optional[int] = None) -> List[Dict[str, Any]]:
        """Busca atividades do usuário."""
        try:
            query = self.db.table("user_activities").select("*").eq("user_id", user_id)
    ...
    def get_user_orders(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            result = self.db.table("orders").select("*").eq("user_id", user_id).order("created_at").execute()
            return result.data if result.data else []
        except (ValueError, KeyError) as e:
            logger.warning(f"Erro de validação: {str(e)}")
            # Tratamento específico pode ser adicionado aqui
    ...
    def get_user_last_login(self, user_id: str) -> Optional[datetime]:
        ...
        try:
            result = self.db.table("users").select("last_login").eq("id", user_id).single().execute()
