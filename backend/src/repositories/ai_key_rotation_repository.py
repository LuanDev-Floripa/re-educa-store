...
class AIKeyRotationRepository(BaseRepository):
    ...
    def create_rotation_log(self, log_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Erro ao criar log de rotação: {str(e)}", exc_info=True)
            return None

    def find_rotation_logs(self, config_id: Optional[str] = None, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        ...
        try:
            ...
            self.logger.error(f"Erro ao buscar logs de rotação: {str(e)}", exc_info=True)
            return []
    ...
    def update_security_setting(self, key: str, value: Any) -> bool:
        ...
        try:
            ...
            result = (
                ...
                .update({"setting_value": str(value), "updated_at": datetime.now().isoformat()})
                .eq("setting_key", key)
                .execute()
            )
            ...
            self.logger.error(f"Erro ao atualizar configuração: {str(e)}", exc_info=True)
            return False
