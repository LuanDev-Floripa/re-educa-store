...
class TwoFactorRepository(BaseRepository):
    ...
    def find_by_user(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Erro ao buscar 2FA do usuário: {str(e)}", exc_info=True)
            return None

    def upsert_config(
        self, user_id: str, secret_key: str, backup_codes: str, enabled: bool = False
        ...
        try:
            ...
            result = self.db.table(self.table_name).upsert(config_data, on_conflict="user_id").execute()
            ...
            self.logger.error(f"Erro ao fazer upsert de 2FA: {str(e)}", exc_info=True)
            return None
    ...
    def update_enabled(self, user_id: str, enabled: bool) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = (
                ...
                .update({"enabled": enabled, "updated_at": datetime.now().isoformat()})
                .eq("user_id", user_id)
                .execute()
            )
            ...
            self.logger.error(f"Erro ao atualizar enabled do 2FA: {str(e)}", exc_info=True)
            return None
    ...
    def update_backup_codes(self, user_id: str, backup_codes: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            result = (
                self.db.table(self.table_name)
                .update({"backup_codes": backup_codes, "updated_at": datetime.now().isoformat()})
                .eq("user_id", user_id)
                .execute()
            )
            ...
            self.logger.error(f"Erro ao atualizar backup codes: {str(e)}", exc_info=True)
            return None
    ...
    def get_secret_key(self, user_id: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.error(f"Erro ao buscar secret key: {str(e)}", exc_info=True)
            return None
    ...
    def is_enabled(self, user_id: str) -> bool:
        ...
        try:
            ...
            self.logger.error(f"Erro ao verificar se 2FA está habilitado: {str(e)}", exc_info=True)
            return False
    ...
    def get_backup_codes(self, user_id: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.error(f"Erro ao buscar backup codes: {str(e)}", exc_info=True)
            return None
