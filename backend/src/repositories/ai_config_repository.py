...
class AIConfigRepository(BaseRepository):
    ...
    def find_by_provider(
        ...
        try:
            ...
            self.logger.error(f"Erro ao buscar configuração por provider: {str(e)}", exc_info=True)
            return None
    ...
    def find_active(self, include_inactive: bool = False) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return self.find_all(filters=filters, order_by="created_at", desc=True)
        except (ValueError, KeyError) as e:
            logger.warning(f"Erro de validação: {str(e)}")
            # Tratamento específico pode ser adicionado aqui
        except Exception as e:
            self.logger.error(f"Erro ao listar configurações: {str(e)}", exc_info=True)
            return []
    ...
    def update_usage_count(self, config_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return self.update(config_id, {"usage_count": new_count, "last_used_at": datetime.now().isoformat()})
        except (ValueError, KeyError) as e:
            logger.warning(f"Erro de validação: {str(e)}")
            # Tratamento específico pode ser adicionado aqui
        except Exception as e:
    ...
    def create_usage_log(self, log_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Erro ao criar log de uso: {str(e)}", exc_info=True)
            return None

    def find_usage_logs(
        self, start_date: Optional[str] = None, provider: Optional[str] = None, limit: Optional[int] = None
    ) -> List[Dict[str, Any]]:
        """
        Busca logs de uso de IA.

        Args:
            start_date: Data inicial (ISO format)
            provider: Provider para filtrar (via join com ai_configurations)
            limit: Limite de resultados

        Returns:
            Lista de logs de uso
        """
        try:
            query = self.db.table("ai_usage_logs").select("*")

            if start_date:
                query = query.gte("created_at", start_date)

            # Se provider especificado, fazer join com ai_configurations
            if provider:
            ...
            self.logger.error(f"Erro ao buscar logs de uso: {str(e)}", exc_info=True)
            return []
