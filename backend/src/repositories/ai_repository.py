...
class AIRepository(BaseRepository):
    ...
    def create_chat_message(self, message_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Erro ao criar mensagem de chat: {str(e)}", exc_info=True)
            return None
    ...
    def find_by_user(self, user_id: str, limit: int = 50) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Erro ao buscar histórico de chat do usuário {user_id}: {str(e)}", exc_info=True)
            return []
    ...
    def find_by_intent(self, user_id: str, intent: str, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            return self.find_all(
                filters={"user_id": user_id, "intent": intent}, order_by="created_at", desc=True, limit=limit
            )
        except (ValueError, KeyError) as e:
            logger.warning(f"Erro de validação: {str(e)}")
            # Tratamento específico pode ser adicionado aqui
        except Exception as e:
            self.logger.error(f"Erro ao buscar mensagens por intenção: {str(e)}", exc_info=True)
            return []
