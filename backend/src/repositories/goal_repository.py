...
class GoalRepository(BaseRepository):
    ...
    def find_by_user(self, user_id: str, is_active: Optional[bool] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"Erro ao buscar metas do usuário {user_id}: {str(e)}", exc_info=True)
            return []
    ...
    def update_progress(self, goal_id: str, progress: int) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            return self.update(goal_id, {"progress": progress, "updated_at": self._get_current_timestamp()})
        except (ValueError, KeyError) as e:
            logger.warning(f"Erro de validação: {str(e)}")
            # Tratamento específico pode ser adicionado aqui
        except Exception as e:
            self.logger.error(f"Erro ao atualizar progresso da meta {goal_id}: {str(e)}", exc_info=True)
            return None
    ...
    def deactivate(self, goal_id: str) -> Optional[Dict[str, Any]]:
        ...
        """
        try:
            return self.update(goal_id, {"is_active": False, "updated_at": self._get_current_timestamp()})
        except (ValueError, KeyError) as e:
            logger.warning(f"Erro de validação: {str(e)}")
            # Tratamento específico pode ser adicionado aqui
        except Exception as e:
            self.logger.error(f"Erro ao desativar meta {goal_id}: {str(e)}", exc_info=True)
            return None
