...
def admin_ai_health():
    ...
    try:
        ...
        health_data = {
            ...
            'available_configs': (
                len(configs_result['data']) if configs_result['success'] else 0
            ),
            'active_configs': (
                len([c for c in configs_result['data'] if c['is_active']])
                if configs_result['success'] else 0
            ),
            'providers': (
                list(set(c['provider'] for c in configs_result['data']))
                if configs_result['success'] else []
            )
        }
