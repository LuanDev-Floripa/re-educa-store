-- ============================================================
-- Migração 008: Sistema de Upload e Gerenciamento de Vídeos
-- Para: RE-EDUCA Store - Sistema de Reeducação de Estilo de Vida
-- Data: 2025
-- ============================================================
--
-- Esta migração consolida:
-- - 07_video_upload_schema.sql
-- - 16_add_video_metadata.sql
--
-- Cria sistema completo de upload, visualização e analytics de vídeos
-- ============================================================
...
DROP POLICY IF EXISTS "Users can view their own videos" ON video_uploads;
CREATE POLICY "Users can view their own videos" ON video_uploads
    FOR SELECT USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can view public videos" ON video_uploads;
CREATE POLICY "Users can view public videos" ON video_uploads
    FOR SELECT USING (status = 'ready');

DROP POLICY IF EXISTS "Users can insert their own videos" ON video_uploads;
CREATE POLICY "Users can insert their own videos" ON video_uploads
    FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own videos" ON video_uploads;
CREATE POLICY "Users can update their own videos" ON video_uploads
    FOR UPDATE USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can delete their own videos" ON video_uploads;
CREATE POLICY "Users can delete their own videos" ON video_uploads
    FOR DELETE USING (auth.uid() = user_id);

-- Políticas RLS para video_views
DROP POLICY IF EXISTS "Users can view video views" ON video_views;
...
DROP POLICY IF EXISTS "Users can view video likes" ON video_likes;
...
DROP POLICY IF EXISTS "Users can view video comments" ON video_comments;
...
DROP POLICY IF EXISTS "Users can view video shares" ON video_shares;
CREATE POLICY "Users can view video shares" ON video_shares
    FOR SELECT USING (true);

DROP POLICY IF EXISTS "Users can insert their own shares" ON video_shares;
...
DROP TRIGGER IF EXISTS update_video_likes_count_trigger ON video_likes;
...
DROP TRIGGER IF EXISTS update_video_comments_count_trigger ON video_comments;
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'video_uploads' AND column_name = 'title') THEN
        ALTER TABLE video_uploads ADD COLUMN title VARCHAR(255);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'video_uploads' AND column_name = 'description') THEN
        ALTER TABLE video_uploads ADD COLUMN description TEXT;
    END IF;
END $$;
...
SELECT 'Migração 008: Sistema de vídeos criado com sucesso!' as status;
